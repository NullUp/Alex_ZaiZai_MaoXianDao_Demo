...
const isJumpAction = (name: string) => name === 'jumpStart' || name === 'jumpEnd'
...
export function HeroModel({ motionRef }: HeroModelProps) {
  ...
  const clips = useMemo(
    ...
    [idle.animations, walk.animations, run.animations, jump.animations],
  )
  ...
  useFrame(() => {
    ...
    const nextAction = !motion?.grounded ? jumpAction : motion.moving ? (motion.running ? 'run' : 'walk') : 'idle'

    if (nextAction !== activeActionRef.current) {
      actions[activeActionRef.current]?.fadeOut(0.18)
      playAction(actions[nextAction], 0.18, isJumpAction(nextAction) ? LoopOnce : LoopRepeat)
      activeActionRef.current = nextAction
    }

    const action = actions[nextAction]
    if (action && !action.isRunning() && !isJumpAction(nextAction)) {
